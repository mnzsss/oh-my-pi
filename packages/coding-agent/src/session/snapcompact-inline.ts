...
export function estimateInlineSavings(input: {
	...
	const budget = snapcompact.providerImageBudget(model.provider, model.imageBudget) - existingImages;
...
export class SnapcompactInlineTransformer {
	...
	async transform(context: Context, model: Model): Promise<Context> {
		...
		const tokenizer = new Tokenizer(model);
		const budget =
			snapcompact.providerImageBudget(model.provider, model.imageBudget) - countMessageImages(context.messages);
